...
if ( ! function_exists( 'kays_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function kays_posted_on() {
	...
	$byline = sprintf(
		esc_html_x( 'Written by %s', 'post author', 'kays' ),
...
if ( ! function_exists( 'kays_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function kays_entry_footer() {
	// Hide tag text for pages.
	if ( 'post' === get_post_type() ) {

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'kays' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged as %1$s', 'kays' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
 ...
 * @return bool
 */
function kays_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'kays_categories' ) ) ) {
		...
		set_transient( 'kays_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so kays_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so kays_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in kays_categorized_blog.
 */
function kays_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'kays_categories' );
}
...
function kays_post_navigation() {
	the_post_navigation( array(
		'next_text' => __( 'Next Post', 'kays' ) . kays_get_svg( array( 'icon' => 'arrow-point-right', 'fallback' => true ) ) . '</span> ' .
			'<span class="screen-reader-text">' . __( 'Next post:', 'kays' ) . '</span> ' .
			'<span class="post-title">%title</span>',
		'prev_text' => kays_get_svg( array( 'icon' => 'arrow-point-left', 'fallback' => true ) ) . __( 'Previous Post', 'kays' ) . '</span> ' .
